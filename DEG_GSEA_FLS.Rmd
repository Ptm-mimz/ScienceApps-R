...
# path to files
# Prepare ranked gene list from your dataset

```{r}

setwd("/data/plapha/TNF DEG-hand_knee")

data <- read.csv("DEG_RA_Hand_TNF_Basal.csv")
...
gene_sets <- list(
  ...
  lipid_met = c("ABCA3", "ABCB11", "ABCD1", "ABCD2", "ABCG1", "ABCG4", "ACER1", "ADGRF1", 
  ...
  mito_ROS = c("COQ9", "MIR210", "MT-ND4L", "NDUFC2-KCTD14", "ALOX12", "BMP7", "BST1", "CBS", 
  ...
  neutral_lipid = c("ABHD12", "ABHD12B", "ABHD16A", "ABHD16B", "ABHD2", "ACSL1", "AGMO", "AGPAT2", 
...
```
Format gene sets for GSEA

```{r}
gmt_df <- stack(gene_sets) %>%
...
gsea_result <- GSEA(
  ...
  TERM2GENE = gmt_df,
  verbose = FALSE,
  pvalueCutoff = 0.1 #0.05 if want to be more strict
)
...
gsea_result <- GSEA(
  geneList = ranked_genes,
  TERM2GENE = gmt_df,
  eps = 0,       # For better p-value estimation of very small p-values
  pvalueCutoff = 1,  # First get ALL results, then filter later
  verbose = TRUE  # See processing details
)
```
Visualize with NES & p.adjust color

```{r}
gsea_df <- as.data.frame(gsea_result)
 
ggplot(gsea_df, aes(x = fct_reorder(ID, NES), y = NES, fill = p.adjust)) +
  geom_col() +
  coord_flip() +
  scale_fill_gradient(low = "firebrick", high = "steelblue", name = "Adjusted p") +
  labs(
    title = "GSEA of Fibroblast functional Gene Sets (Hand_TNF vs Basal)",
